# 🧠 ML Unlearning Research

A personal research knowledge base on machine unlearning — exact, certified/approximate, and LLM knowledge unlearning — built with Obsidian + Claude (via MCP).
...
├── 03-Architectures/  → Model families, methods & specific systems
...
Reusable building blocks that papers reference: Certified Removal, Influence Functions, Differential Privacy, SISA, Deletion Capacity, Membership Inference, etc. One concept = one note. Papers link *into* this folder via `[[wikilinks]]`. Over time this becomes a personal unlearning encyclopedia.

### 03-Architectures — Specific models & methods
One note per method or system: SISA, Descent-to-Delete, Certified-Removal, gradient-ascent forgetting, benchmark suites (TOFU, WMDP, MUSE, RWKU). Different from concepts — this is about *specific methods/systems*, their mechanics and design choices.

### 04-Experiments — Lab notebook
Hands-on work: reproductions, deletion-vs-retrain benchmarks, recovery-attack tests. Tracks hypothesis → setup → results → conclusion. Even *planned* experiments live here.

### 05-Glossary — Quick reference
2-3 sentence definitions only. Link here when you need a fast reminder of what "(ε,δ)-indistinguishability" or "deletion capacity" means without reading a full concept note.

### 06-Critical-Reviews — Comparative judgment layer
Human + agent reviews of methods, guarantees, and paper clusters. These notes are explicitly analytical: compare alternatives, surface tradeoffs, identify failure modes, and record your own engineering/research judgment. Use this folder after the source notes are already processed and, ideally, reviewed.
...
This vault is connected to Claude through the Obsidian Local REST API MCP server. Claude can read, create, and update notes directly. Below are battle-tested prompt patterns.

### 1. Process a Paper (most common)

**Just drop the PDF or paste the abstract + arXiv link.** Then say:

> Add this paper to my unlearning wiki. Process it into `01-Papers/` using the template, and create or update related notes in `02-Concepts/`, `03-Architectures/`, `04-Experiments/`, and `05-Glossary/` if applicable.
...
> The note `02-Concepts/Certified Removal.md` is a stub. Expand it with: intuition, math, common variants, and link to relevant papers in `01-Papers/`.
...
> Use `agents/critical-reviewer.md`. Compare the methods in `01-Papers/<paper A>.md` and `01-Papers/<paper B>.md`. Create a critical review in `06-Critical-Reviews/` analyzing differences in: thesis, method, assumptions, guarantee, evaluation, results, tradeoffs, and engineering takeaways.

### 7. Plan an Experiment

> Based on `01-Papers/<paper>.md`, draft an experiment plan in `04-Experiments/` to reproduce the main result. Include: hypothesis, setup, method, expected results table, risks.

### 8. Build a Topic Cluster

When starting a new research area:

> I want to study certified unlearning. Create stub notes in `02-Concepts/` for: Certified Removal, Differential Privacy, Influence Functions, Deletion Capacity, and Membership Inference. Each should follow the template with at least one paragraph in the Definition and Intuition sections.
...
- **Critical Reviewer** — runs after source notes are reviewed; compares methods/guarantees and records opinionated insight in `06-Critical-Reviews/`.
...
> Use `agents/critical-reviewer.md`. Compare `01-Papers/<paper A>.md`, `01-Papers/<paper B>.md`, and any relevant method/concept notes. Create `06-Critical-Reviews/<review title>.md`.
...
**Bad:** "Talk about certified removal"
**Good:** "Expand `02-Concepts/Certified Removal.md`, link to relevant papers in `01-Papers/`"
...
**Good:** "Compare in a markdown table with columns: Method, Guarantee, Strength, Weakness"
...
| `exact-unlearning` | SISA, sharded retraining, provably-identical methods |
| `certified` | (ε,δ) certified / approximate unlearning |
| `llm-unlearning` | LLM knowledge / concept unlearning |
| `privacy` | Differential privacy, right-to-be-forgotten, GDPR |
| `verification` | Auditing, proofs, membership inference, recovery attacks |
| `evaluation` | Benchmarks (TOFU, WMDP, MUSE, RWKU), metrics |
...
- **Exact unlearning** — SISA, sharded/sliced retraining
- **Certified / approximate unlearning** — (ε,δ) guarantees, influence-function updates
- **Deep-net frontier** — extending certificates to non-convex models
- **LLM knowledge unlearning** — concept/PII/capability removal, benchmarks
- **Verification & robustness** — auditable deletion, recovery attacks, sequential deletion
...
Copy these into Claude when you need them:

```
"Process the attached paper into my unlearning wiki"
...
- **Claude** — connects to vault via MCP
