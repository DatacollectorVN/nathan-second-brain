...
- **Distribution-aware sharding (§VIII, Algorithm 1 — optional):** when the unlearning-request distribution is known and stable, replace uniform sharding. Each owner $u$ has erasure probability $p(u)$; requests hitting shard $D_i$ form a Poisson binomial $\chi_i$ with $\mathbb{E}(\chi_i) = n\bar{p}$, where $n$ = points in the shard and $\bar{p}$ = mean $p(u)$ within it — keeping $\mathbb{E}(\chi_i) < C \le 1$ means the shard is, in expectation, never retrained. *Algorithm 1:* sort points by $p(u)$, fill the current shard lowest-probability-first until adding a point would push $\mathbb{E}(\chi_i) \ge C$, then close it and start the next with the residual data — high-risk data ends up concentrated in few, small shards; low-risk data in few, large ones. In the paper's 3-country scenario (per-country $p_c \in \{3\times10^{-6}, 3\times10^{-5}, 6\times10^{-6}\}$, modeled on Google's reported deletion rates) this yields 19 shards on SVHN with fewer expected retrained points, costing ~1 PP accuracy (94.4% vs. 95.7% uniform) because unequal shard sizes strain majority voting. Fairness caveat: clustering likely-to-unlearn populations may degrade predictions for exactly those groups.


![[statics/sisa-alg1-distribution-aware-sharding.png|450]]
*Algorithm 1 (ShardData) from [[Machine Unlearning (SISA)]] — see that note for a line-by-line walkthrough.*
